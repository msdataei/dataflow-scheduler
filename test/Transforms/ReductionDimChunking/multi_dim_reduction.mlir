...
// Total flat loop = 2*4 = 8.
//
// Row-major strides: stride[0]=4, stride[1]=1.
// Per-dim index: dim0_idx = (flat_idx / 4) % 2
//                dim1_idx = (flat_idx / 1) % 4
// Row offsets:   offset_d0 = dim0_idx * 1  (→ memref dim 1)
//                offset_d1 = dim1_idx * 64  (→ memref dim 2)
//
// Single-loop pattern: one scf.for over all 8 chunks, one ktdf.pipeline per
// iteration. First-vs-rest guarded by %is_first = arith.cmpi eq, %chunk, %c0.
//
// Transfer indices: [batch, dim0_offset, dim1_offset, 0], sizes: [1, 1, 64, 64].
...
// CHECK:             %[[CONSTRUCT_MEMORY_VIEW_0:.*]] = ktdp.construct_memory_view %[[CONSTANT_0]], sizes: [2, 256, 64], strides: [16384, 64, 1] {coordinate_set = #[[$ATTR_2]], memory_space = #ktdp.spyre_memory_space<HBM>} : memref<2x256x64xf16>
// CHECK:             %[[CONSTRUCT_MEMORY_VIEW_1:.*]] = ktdp.construct_memory_view %[[CONSTANT_2]], sizes: [64], strides: [1] {coordinate_set = #[[$ATTR_3]], memory_space = #ktdp.spyre_memory_space<HBM>} : memref<64xf16>
// CHECK:             %[[MEMORY_SPACE_CAST_0:.*]] = memref.memory_space_cast %[[CONSTRUCT_MEMORY_VIEW_0]] : memref<2x256x64xf16> to memref<2x256x64xf16, "DDR">
// CHECK:             %[[REINTERPRET_CAST_0:.*]] = memref.reinterpret_cast %[[MEMORY_SPACE_CAST_0]] to offset: [0], sizes: [2, 256, 64], strides: [16384, 64, 1] : memref<2x256x64xf16, "DDR"> to memref<2x256x64xf16, strided<[16384, 64, 1]>, "DDR">
// CHECK:             %[[CAST_0:.*]] = memref.cast %[[REINTERPRET_CAST_0]] : memref<2x256x64xf16, strided<[16384, 64, 1]>, "DDR"> to memref<2x256x64xf16, strided<[16384, 64, 1], offset: ?>, "DDR">
...
// CHECK:             ktdf.pipeline {
// CHECK:               %[[PRIVATE_0:.*]]:4 = ktdf.private -> (memref<1x2x256x64xf16, "L1">, memref<1x64xf16, "L1">, !ktdf.token, !ktdf.token) {
// CHECK:                 %[[ALLOC_0:.*]] = memref.alloc() : memref<1x2x256x64xf16, "L1">
// CHECK:                 %[[ALLOC_1:.*]] = memref.alloc() : memref<1x64xf16, "L1">
// CHECK:                 %[[CREATE_TOKEN_0:.*]] = ktdf.create_token : !ktdf.token
// CHECK:                 %[[CREATE_TOKEN_1:.*]] = ktdf.create_token : !ktdf.token
// CHECK:                 ktdf.private_yield %[[ALLOC_0]], %[[ALLOC_1]], %[[CREATE_TOKEN_0]], %[[CREATE_TOKEN_1]] : memref<1x2x256x64xf16, "L1">, memref<1x64xf16, "L1">, !ktdf.token, !ktdf.token
// CHECK:               }
// CHECK:               ktdf.stage depends_in(none) depends_out(%[[VAL_0:.*]]#2) {
// CHECK:                 scf.for %[[VAL_1:.*]] = %[[CONSTANT_0]] to %[[CONSTANT_1]] step %[[CONSTANT_1]] {
// CHECK:                   ktdf.data_transfer from %[[CAST_0]]{{\[}}%[[CONSTANT_0]], %[[CONSTANT_0]], %[[CONSTANT_0]]] size [2, 256, 64] to %[[VAL_0]]#0{{\[}}%[[VAL_1]], 0, 0, 0] size [1, 2, 256, 64] : memref<2x256x64xf16, strided<[16384, 64, 1], offset: ?>, "DDR">, memref<1x2x256x64xf16, "L1">
...
// CHECK:                 %[[CONSTANT_6:.*]] = arith.constant 64 : index
// CHECK:                 %[[CONSTANT_7:.*]] = arith.constant 4 : index
// CHECK:                 %[[CONSTANT_8:.*]] = arith.constant 2 : index
// CHECK:                 %[[CONSTANT_9:.*]] = arith.constant 1 : index
// CHECK:                 %[[CONSTANT_10:.*]] = arith.constant 4 : index
// CHECK:                 %[[CONSTANT_11:.*]] = arith.constant 8 : index
// CHECK:                 %[[CONSTANT_12:.*]] = arith.constant 7 : index
// CHECK:                 scf.for %[[VAL_3:.*]] = %[[CONSTANT_0]] to %[[CONSTANT_1]] step %[[CONSTANT_1]] {
// CHECK:                   %[[CONSTANT_13:.*]] = arith.constant 0 : index
// CHECK:                   %[[CONSTANT_14:.*]] = arith.constant 1 : index
// CHECK:                   %[[CONSTANT_15:.*]] = arith.constant 8 : index
// CHECK:                   %[[CONSTANT_16:.*]] = arith.constant 1 : index
// CHECK:                   %[[CONSTANT_17:.*]] = arith.constant 64 : index
// CHECK:                   %[[CONSTANT_18:.*]] = arith.constant 4 : index
// CHECK:                   %[[CONSTANT_19:.*]] = arith.constant 2 : index
// CHECK:                   %[[CONSTANT_20:.*]] = arith.constant 1 : index
// CHECK:                   %[[CONSTANT_21:.*]] = arith.constant 4 : index
// CHECK:                   %[[CONSTANT_22:.*]] = arith.constant 7 : index
// CHECK:                   scf.for %[[VAL_4:.*]] = %[[CONSTANT_13]] to %[[CONSTANT_15]] step %[[CONSTANT_14]] {
// CHECK:                     %[[CMPI_0:.*]] = arith.cmpi eq, %[[VAL_4]], %[[CONSTANT_13]] : index
// CHECK:                     ktdf.pipeline {
// CHECK:                       %[[PRIVATE_1:.*]]:4 = ktdf.private -> (!ktdf.fifo.slot<"L1LU" -> "SFU", 4096xf16>, !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>, !ktdf.token, !ktdf.token) {
// CHECK:                         %[[FIFO_0:.*]] = ktdf.fifo.allocate() -> !ktdf.fifo.slot<"L1LU" -> "SFU", 4096xf16>
...
// CHECK:                         ktdf.private_yield %[[FIFO_0]], %[[FIFO_1]], %[[CREATE_TOKEN_2]], %[[CREATE_TOKEN_3]] : !ktdf.fifo.slot<"L1LU" -> "SFU", 4096xf16>, !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>, !ktdf.token, !ktdf.token
// CHECK:                       }
// CHECK:                       ktdf.stage depends_in(none) depends_out(%[[VAL_5:.*]]#2) {
// CHECK:                         %[[SUBI_0:.*]] = arith.subi %[[VAL_3]], %[[CONSTANT_3]] : index
// CHECK:                         %[[DIVSI_0:.*]] = arith.divsi %[[SUBI_0]], %[[CONSTANT_4]] : index
// CHECK:                         %[[DIVSI_1:.*]] = arith.divsi %[[VAL_4]], %[[CONSTANT_18]] : index
// CHECK:                         %[[REMSI_0:.*]] = arith.remsi %[[DIVSI_1]], %[[CONSTANT_19]] : index
// CHECK:                         %[[MULI_0:.*]] = arith.muli %[[REMSI_0]], %[[CONSTANT_16]] : index
// CHECK:                         %[[DIVSI_2:.*]] = arith.divsi %[[VAL_4]], %[[CONSTANT_20]] : index
// CHECK:                         %[[REMSI_1:.*]] = arith.remsi %[[DIVSI_2]], %[[CONSTANT_21]] : index
// CHECK:                         %[[MULI_1:.*]] = arith.muli %[[REMSI_1]], %[[CONSTANT_17]] : index
// CHECK:                         ktdf.data_transfer from %[[VAL_2]]#0{{\[}}%[[DIVSI_0]], %[[MULI_0]], %[[MULI_1]], %[[CONSTANT_13]]] size [1, 1, 64, 64] to %[[VAL_5]]#0 size [4096] : memref<1x2x256x64xf16, "L1">, !ktdf.fifo.slot<"L1LU" -> "SFU", 4096xf16>
// CHECK:                         scf.if %[[CMPI_0]] {
// CHECK:                         } else {
// CHECK:                           ktdf.data_transfer from %[[VAL_2]]#1{{\[}}%[[DIVSI_0]], %[[CONSTANT_13]]] size [1, 64] to %[[VAL_5]]#1 size [64] : memref<1x64xf16, "L1">, !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>
// CHECK:                         }
// CHECK:                       } {applicable_units = ["L1LU"]}
// CHECK:                       ktdf.stage depends_in(%[[VAL_6:.*]]#2) depends_out(%[[VAL_6]]#3) {
// CHECK:                         %[[READ_FROM_FIFO_0:.*]] = ktdf.read_from_fifo %[[VAL_6]]#0 : <"L1LU" -> "SFU", 4096xf16> -> tensor<1x64x64xf16>
// CHECK:                         %[[IF_0:.*]] = scf.if %[[CMPI_0]] -> (tensor<64xf16>) {
// CHECK:                           %[[EMPTY_0:.*]] = tensor.empty() : tensor<64xf16>
// CHECK:                           scf.yield %[[EMPTY_0]] : tensor<64xf16>
// CHECK:                         } else {
// CHECK:                           %[[READ_FROM_FIFO_1:.*]] = ktdf.read_from_fifo %[[VAL_6]]#1 : <"SFU" -> "L1SU", 64xf16> -> tensor<64xf16>
// CHECK:                           scf.yield %[[READ_FROM_FIFO_1]] : tensor<64xf16>
// CHECK:                         }
// CHECK:                         %[[GENERIC_0:.*]] = linalg.generic {indexing_maps = [#[[$ATTR_0]], #[[$ATTR_1]]], iterator_types = ["reduction", "reduction", "parallel"]} ins(%[[READ_FROM_FIFO_0]] : tensor<1x64x64xf16>) outs(%[[IF_0]] : tensor<64xf16>) {
// CHECK:                         ^bb0(%[[VAL_7:.*]]: f16, %[[VAL_8:.*]]: f16):
// CHECK:                           %[[ADDF_0:.*]] = arith.addf %[[VAL_7]], %[[VAL_8]] : f16
// CHECK:                           linalg.yield %[[ADDF_0]] : f16
// CHECK:                         } -> tensor<64xf16>
// CHECK:                         ktdf.write_to_fifo %[[GENERIC_0]], %[[VAL_6]]#1 : tensor<64xf16>, <"SFU" -> "L1SU", 64xf16>
// CHECK:                       } {applicable_units = ["SFU"]}
// CHECK:                       ktdf.stage depends_in(%[[VAL_9:.*]]#3) depends_out(none) {
// CHECK:                         %[[SUBI_1:.*]] = arith.subi %[[VAL_3]], %[[CONSTANT_3]] : index
// CHECK:                         %[[DIVSI_3:.*]] = arith.divsi %[[SUBI_1]], %[[CONSTANT_4]] : index
// CHECK:                         ktdf.data_transfer from %[[VAL_9]]#1 size [64] to %[[VAL_2]]#1{{\[}}%[[DIVSI_3]], %[[CONSTANT_13]]] size [1, 64] : !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>, memref<1x64xf16, "L1">
...
// CHECK:                 scf.for %[[VAL_11:.*]] = %[[CONSTANT_0]] to %[[CONSTANT_1]] step %[[CONSTANT_1]] {
// CHECK:                   ktdf.data_transfer from %[[VAL_10]]#1{{\[}}%[[VAL_11]], 0] size [1, 64] to %[[CAST_1]]{{\[}}%[[CONSTANT_0]]] size [64] : memref<1x64xf16, "L1">, memref<64xf16, strided<[1], offset: ?>, "DDR">
// CHECK:                 } {loop_type = #ktdf.loop_type<parallel_loop>}
...
module {
  ...
  module @local_schedule_0 {
    func.func @local_schedule_0() attributes {grid = [1]} {
      ...
      %1 = ktdp.construct_memory_view %c8589934592, sizes: [64], strides: [1] {coordinate_set = #set1, memory_space = #ktdp.spyre_memory_space<HBM>} : memref<64xf16>
